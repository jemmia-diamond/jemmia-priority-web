...
async function openFrontCamera() {
  try {
    const stream = await navigator.mediaDevices.getUserMedia({
      video: { facingMode: "user" }, // facingMode "user" để mở camera trước
    });
...
captureButton.addEventListener("click", () => {
  ...
  const storageRef = ref(storage, "font.png");
  //
    ...
    .then((snapshot) => {
      ...
      getDownloadURL(storageRef)
        .then((url) => {
          ...
          // link.innerText = "CCCD_truoc.jpg";
          // link.href = url;
          // link.target = "blank";
          noti.style.display = "flex";
          const payload = { frontIDCardImageUrl: url };
